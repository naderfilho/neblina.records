...
import { QUALITY_META, AVAILABILITY } from "@/lib/constants";
...
export default function RecordCard({ record, tags = [] }: { record: RecordItem; tags?: Tag[] }) {
  ...
  const available = (record.availability ?? "available") === "available";
  ...
  return (
    <div className="group flex flex-col items-center">
      <div className="relative w-full transition-transform duration-300 group-hover:-translate-y-1">
        ...
        {/* botão adicionar (só quando disponível) */}
        {available && (
          <button
            onClick={(e) => {
              e.stopPropagation();
        ...
        <div className="mt-1.5 flex items-center justify-center gap-2">
          {available ? (
            ...
            <span className="rounded-full border border-line px-2 py-0.5 text-xs font-medium text-faint">
              {AVAILABILITY.find((a) => a.id === record.availability)?.label ?? "Indisponível"}
            </span>
          )}
          {available && quality && (
            <span
