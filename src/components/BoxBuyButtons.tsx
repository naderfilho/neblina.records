...
export default function BoxBuyButtons({
  id, title, subtitle, price, coverUrl, available = true,
}: {
  id: string;
  title: string;
  ...
  if (!available) {
    return (
      <div className="rounded-xl border border-line bg-bg-soft px-4 py-3 text-sm text-muted">
        Este box está <strong className="text-mist">indisponível</strong> no momento.
      </div>
    );
  }
  ...
  async function getName(): Promise<string | null> {
    ...
    if (!user) { router.push(`/login?next=/box/${id}`); return null; }
  ...
  function message(name: string) {
    ...
    const link = `${STORE.siteUrl}/box/${id}`;
    return `Olá, Neblina Records! ${hello} interesse no seguinte box:\n\n${title}${subtitle ? ` — ${subtitle}` : ""} — ${formatBRL(price)}\n${link}`;
  }
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        style={{ background: "linear-gradient(135deg, #1faf54, #12924a)", boxShadow: "0 5px 14px -8px rgba(18,146,74,0.45)" }}
      >
        <WhatsAppGlyph size={16} /> Comprar o box pelo WhatsApp
      </button>

      <button
        onClick={() => (inCart ? cart.setOpen(true) : cart.add({ id, title, artist: subtitle ?? "Box", price, coverUrl }))}
        className={`flex w-fit items-center gap-1.5 rounded-lg border px-3 py-2 text-xs transition-colors ${
          inCart ? "border-teal/60 bg-teal/15 text-teal" : "border-line bg-panel text-muted hover:border-brand/50 hover:text-ink"
        }`}
      >
